...
func (p *Postgres) GetKReceiptDeduction() (Deduction, error ){
	
	rows, err := p.Db.Query("select * from personal_deduct where id=2")
	// dd := Deduction{}
	if err != nil {
		return Deduction{}, err 
	}
...
func (p *Postgres) PostKReceiptDeduction(amount float64) (Deduction, error){

	created_time := time.Now()
	result, err := p.Db.Exec(`Insert into personal_deduct(id, type, amount, created_at) values(2, $1, $2, $3)`, "k-receipt", amount, created_time)
	var dd Deduction
	if err != nil {
		return dd, err 
	}
...
func (p *Postgres) UpdateKReceiptDeduction(amount float64) (Deduction, error){

	created_time := time.Now()
	result, err := p.Db.Exec(`Update personal_deduct set amount=$1, created_at=$2 where id=2`, amount, created_time)
	var dd Deduction
	if err != nil {
		return dd, err 
	}
